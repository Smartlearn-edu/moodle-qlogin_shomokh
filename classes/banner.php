...
final class banner {
    ...
    public static function render(): string {
        global $PAGE, $USER;
        if (!isloggedin() || isguestuser() || empty($USER->id) || is_siteadmin($USER->id)
        ...
        if (migration::can_self_claim($USER)) {
            ...
            return \html_writer::tag('aside',
                \html_writer::span(get_string($key, 'local_qlogin_shomokh', userdate($deadline)),
                    'local-qlogin-shomokh-banner__message')
                    . \html_writer::link(new \moodle_url('/local/qlogin_shomokh/link_existing.php'),
        ...
        return \html_writer::tag('aside',
            \html_writer::span(get_string($key, 'local_qlogin_shomokh', $values),
                'local-qlogin-shomokh-banner__message')
                . \html_writer::link(new \moodle_url('/local/qlogin_shomokh/verify.php'),
