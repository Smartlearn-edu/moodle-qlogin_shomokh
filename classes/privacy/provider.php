...
final class provider implements
    ...
    public static function get_metadata(collection $collection): collection {
        $collection->add_database_table('local_qlogin_shomokh_verify',
            [
        ...
        $collection->add_database_table('local_qlogin_shomokh_event',
            [
        ...
        $collection->add_database_table('local_qlogin_shomokh_recov',
            [
        ...
        $collection->add_database_table('local_qlogin_shomokh_lock',
            [
        ...
        $collection->add_database_table('local_qlogin_shomokh_enlock',
            [
            ...
            'privacy:metadata:restrictions');
        $collection->add_database_table('local_qlogin_shomokh_exempt',
            [
        ...
        $collection->add_database_table('local_qlogin_shomokh_mail',
            [
        ...
        $collection->add_database_table('local_qlogin_shomokh_reset',
            [
        ...
        $collection->add_database_table('local_qlogin_shomokh_alias',
            [
    ...
    public static function get_contexts_for_userid($userid): contextlist {
        ...
        foreach (['local_qlogin_shomokh_verify', 'local_qlogin_shomokh_event',
        ...
        if ($DB->record_exists('local_qlogin_shomokh_exempt', ['scope' => 'user', 'scopeid' => $userid])
    ...
    public static function export_user_data(approved_contextlist $contextlist): void {
        ...
        $verification = $DB->get_records('local_qlogin_shomokh_verify', ['userid' => $userid], '',
            'id, channel, target, state, expiresat, verifiedat, timecreated, timemodified');
        $recovery = $DB->get_records('local_qlogin_shomokh_recov', ['userid' => $userid], '',
            'id, phone, state, expiresat, verifiedat, attempts, timecreated, timemodified');
        $events = $DB->get_records('local_qlogin_shomokh_event', ['userid' => $userid], '',
            'id, messageid, eventtype, status, timecreated');
        $exemptions = $DB->get_records_select('local_qlogin_shomokh_exempt',
        ...
        $accountlocks = $DB->get_records('local_qlogin_shomokh_lock', ['userid' => $userid], '',
            'id, previoussuspended, reason, timecreated');
        $enrolmentlocks = $DB->get_records('local_qlogin_shomokh_enlock', ['userid' => $userid], '',
            'id, userenrolid, previousstatus, timecreated');
        $mail = $DB->get_records('local_qlogin_shomokh_mail', ['userid' => $userid], '',
            'id, purpose, provider, recipienthint, status, httpstatus, attempts, timecreated, timemodified');
        $emailreset = $DB->get_records('local_qlogin_shomokh_reset', ['userid' => $userid], '',
            'id, channel, state, expiresat, attempts, timecreated, timemodified');
        $aliases = $DB->get_records_select('local_qlogin_shomokh_alias',
            'userid = :userid OR createdby = :createdby', ['userid' => $userid, 'createdby' => $userid], '',
    ...
    public static function delete_data_for_all_users_in_context(\context $context): void {
        ...
        foreach (['local_qlogin_shomokh_event', 'local_qlogin_shomokh_recov',
